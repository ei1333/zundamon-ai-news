...
`## Theme` は必須で、`config/themes/<name>.json` に対応する theme 名を指定します。現在は `ai` / `shogi` / `vocaloid` を使えます。
...
トップページの絞り込みは、細かいタグではなく **episode の `Theme` (`ai` / `shogi` / `vocaloid`)** を軸にします。
