...
- `config/themes/tech.json` - Techニュース向けテーマ例
- `config/themes/business.json` - Businessニュース向けテーマ例
- `config/themes/game.json` - ゲームニュース向けテーマ例
...
読み上げ用の固有名詞・ブランド名・製品名は、表示用の英字や記号をそのまま残すより、**カタカナまたはひらがなで読みを直接書く** 方針です。
たとえば `Pokémon` は `ポケモン`、`Pokémon HOME` は `ポケモンホーム` のように、台本では読みを優先します。
...
- `# タイトル`
- `## Theme` (`ai` / `tech` / `business` / `game` / `shogi` / `vocaloid` / `sports` など)
...
`## Theme` は必須で、`config/themes/<name>.json` に対応する theme 名を指定します。現在は `ai` / `tech` / `business` / `game` / `shogi` / `vocaloid` / `sports` を使えます。
...
少なくとも **再生成元になる原稿・台本は `main` に残す** のを推奨します。
最低限、次は commit / push 対象に含めます。

- `episodes/YYYY-MM-DD.md`
- `scripts_text/YYYY-MM-DD.txt`
- `scripts_text/YYYY-MM-DD.parts.json`
- 必要に応じて `index.html`
- 必要に応じて `sitemap.xml`
...
```bash
git add episodes/2026-03-13.md scripts_text/2026-03-13.txt scripts_text/2026-03-13.parts.json index.html sitemap.xml
git commit -m "Add episode sources for 2026-03-13"
...
トップページの絞り込みは、細かいタグではなく **episode の `Theme` (`ai` / `tech` / `business` / `game` / `shogi` / `vocaloid` / `sports`)** を軸にします。
