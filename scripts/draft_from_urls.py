...
from draft_models import DraftItem, ResolvedDraftContext
...
def resolve_context(args: argparse.Namespace) -> ResolvedDraftContext:
...
def main() -> None:
    ...
    for idx, url in enumerate(args.urls, start=1):
        fallback_category = context.default_categories[idx - 1]
        try:
            html_text = fetch_url(url)
            raw_title = extract_title(html_text)
            description = extract_description(html_text)
            title = finalize_headline(raw_title, context.theme_name, description=description)
            site_name = extract_site_name(html_text, url)
            primary_tag = infer_category(title, description, fallback_category, context.category_rules)
            items.append(
                DraftItem(
                    headline=title,
                    summary=description,
                    ...
                    tags=infer_tags(title, description, primary_tag, context.tag_rules),
                )
            )
        except (urllib.error.URLError, TimeoutError, socket.timeout) as exc:
            items.append(fallback_from_url(url, fallback_category, str(exc), context.draft_theme, theme_name=context.theme_name))
